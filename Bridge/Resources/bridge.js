...
window.Bridge={emptyFn:function(){},copy:function(to,from,keys,toIf){if(typeof keys==='string'){keys=keys.split(/[,;\s]+/);}
...
return to;},ns:function(ns,scope){var nsParts=ns.split('.');if(!scope){scope=window;}
for(i=0;i<nsParts.length;i++){if(typeof scope[nsParts[i]]=='undefined'){scope[nsParts[i]]={};}
scope=scope[nsParts[i]];}
return scope;},ready:function(fn){if(typeof window.jQuery!=='undefined'){$(fn);}else{Bridge.on('DOMContentLoaded',document,fn);}},on:function(event,elem,fn){function listenHandler(e){var ret=fn.apply(this,arguments);if(ret===false){e.stopPropagation();e.preventDefault();}
return(ret);}
...
Bridge.String={isNullOrWhiteSpace:function(value){return value===null||value.match(/^ *$/)!==null;},isNullOrEmpty:function(value){return Bridge.isEmpty(value,false);},fromCharCount:function(c,count){if(count>=0){return String(Array(count+1).join(String.fromCharCode(c)));}
...
if(this.$constructor){this.$constructor.apply(this,arguments);}};Bridge.Class.initConfig=function(extend,base,cfg,statics,scope){scope.$initMembers=function(){var name,config;config=Bridge.isFunction(cfg)?cfg():cfg;if(extend&&!statics&&base.$initMembers){base.$initMembers.apply(this,arguments);}
if(config.fields){for(name in config.fields){this[name]=config.fields[name];}}
if(config.properties){for(name in config.properties){this[name]=config.properties[name];var rs=name.charAt(0)=="$",cap=rs?name.slice(1):name;this["get"+cap]=(function(name){return function(){return this[name];};})(name);this["set"+cap]=(function(name){return function(value){this[name]=value;};})(name);}}
if(config.events){for(name in config.events){this[name]=config.events[name];var rs=name.charAt(0)=="$",cap=rs?name.slice(1):name;this["add"+cap]=(function(name){return function(value){this[name]=Bridge.fn.combine(this[name],value);};})(name);this["remove"+cap]=(function(name){return function(value){this[name]=Bridge.fn.remove(this[name],value);};})(name);}}
if(config.alias){for(name in config.alias){if(this[name]){this[name]=this[config.alias[name]];}}}
if(config.init){config.init.apply(this,arguments);}};};Bridge.Class.define=function(className,prop){var extend=prop.$extend||prop.extend,statics=prop.$statics||prop.statics,base=extend?extend[0].prototype:this.prototype,prototype,nameParts,scope=prop.$scope||window,i,v,ctorCounter,isCtor,name;if(Bridge.isFunction(extend)){extend=null;}
else if(prop.$extend){delete prop.$extend;}
else{delete prop.extend;}
...
initializing=true;prototype=extend?new extend[0]():new Object();initializing=false;if(statics){var staticsConfig=statics.$config;if(staticsConfig){Bridge.Class.initConfig(extend,base,staticsConfig,true,Class);if(statics.$config){delete statics.$config;}
else{delete statics.config;}}}
var instanceConfig=prop.$config;if(instanceConfig){Bridge.Class.initConfig(extend,base,instanceConfig,false,prop);if(prop.$config){delete prop.$config;}
...
prototype.$$name=className;Class.prototype=prototype;Class.prototype.constructor=Class;Class.$$name=className;if(statics){for(name in statics){Class[name]=statics[name];}}
...
if(Class.$initMembers){Class.$initMembers.call(Class);}
if(Class.constructor){Class.constructor.call(Class);}
return Class;};Bridge.Class.addExtend=function(cls,extend){Array.prototype.push.apply(cls.$$extend,extend);for(i=0;i<extend.length;i++){scope=extend[i];if(!scope.$$inheritors){scope.$$inheritors=[];}
scope.$$inheritors.push(cls);}};Bridge.Class.set=function(scope,className,cls){var nameParts=className.split('.');for(i=0;i<(nameParts.length-1);i++){if(typeof scope[nameParts[i]]=='undefined'){scope[nameParts[i]]={};}
...
else{return new Bridge.Exception(error?error.toString():null);}}}});Bridge.Class.define('Bridge.ErrorException',{extend:[Bridge.Exception],constructor:function(error){Bridge.Exception.prototype.$constructor.call(this,error.message);this.errorStack=error;this.error=error;},getError:function(){return this.error;}});Bridge.Class.define('Bridge.ArgumentException',{extend:[Bridge.Exception],constructor:function(message,paramName,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"Value does not fall within the expected range.",innerException);this.paramName=paramName;},getParamName:function(){return this.paramName;}});Bridge.Class.define('Bridge.ArgumentNullException',{extend:[Bridge.ArgumentException],constructor:function(paramName,message,innerException){if(!message){message='Value cannot be null.';if(paramName){message+='\nParameter name: '+paramName;}}
Bridge.ArgumentException.prototype.$constructor.call(this,message,paramName,innerException);}});Bridge.Class.define('Bridge.ArgumentOutOfRangeException',{extend:[Bridge.ArgumentException],constructor:function(paramName,message,innerException,actualValue){if(!message){message='Value is out of range.';if(paramName){message+='\nParameter name: '+paramName;}}
Bridge.ArgumentException.prototype.$constructor.call(this,message,paramName,innerException);this.actualValue=actualValue;},getActualValue:function(){return this.actualValue;}});Bridge.Class.define('Bridge.CultureNotFoundException',{extend:[Bridge.ArgumentException],constructor:function(paramName,invalidCultureName,message,innerException){if(!message){message='Culture is not supported.';if(paramName){message+='\nParameter name: '+paramName;}
if(invalidCultureName){message+='\n'+invalidCultureName+' is an invalid culture identifier.';}}
Bridge.ArgumentException.prototype.$constructor.call(this,message,paramName,innerException);this.invalidCultureName=invalidCultureName;},getInvalidCultureName:function(){return this.invalidCultureName;}});Bridge.Class.define('Bridge.KeyNotFoundException',{extend:[Bridge.Exception],constructor:function(message,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"Key not found.",innerException);}});Bridge.Class.define('Bridge.ArithmeticException',{extend:[Bridge.Exception],constructor:function(message,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"Overflow or underflow in the arithmetic operation.",innerException);}});Bridge.Class.define('Bridge.DivideByZeroException',{extend:[Bridge.ArithmeticException],constructor:function(message,innerException){Bridge.ArithmeticException.prototype.$constructor.call(this,message||"Division by 0.",innerException);}});Bridge.Class.define('Bridge.OverflowException',{extend:[Bridge.ArithmeticException],constructor:function(message,innerException){Bridge.ArithmeticException.prototype.$constructor.call(this,message||"Arithmetic operation resulted in an overflow.",innerException);}});Bridge.Class.define('Bridge.FormatException',{extend:[Bridge.Exception],constructor:function(message,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"Invalid format.",innerException);}});Bridge.Class.define('Bridge.InvalidCastException',{extend:[Bridge.Exception],constructor:function(message,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"The cast is not valid.",innerException);}});Bridge.Class.define('Bridge.InvalidOperationException',{extend:[Bridge.Exception],constructor:function(message,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"Operation is not valid due to the current state of the object.",innerException);}});Bridge.Class.define('Bridge.NotImplementedException',{extend:[Bridge.Exception],constructor:function(message,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"The method or operation is not implemented.",innerException);}});Bridge.Class.define('Bridge.NotSupportedException',{extend:[Bridge.Exception],constructor:function(message,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"Specified method is not supported.",innerException);}});Bridge.Class.define('Bridge.NullReferenceException',{extend:[Bridge.Exception],constructor:function(message,innerException){Bridge.Exception.prototype.$constructor.call(this,message||"Object is null.",innerException);}});

Bridge.Class.define('Bridge.IFormattable',{statics:{$is:function(obj){if(Bridge.isNumber(obj)){return true;}
if(Bridge.isDate(obj)){return true;}
return Bridge.is(obj,Bridge.IFormattable,true);}}});Bridge.Class.define('Bridge.IComparable',{});Bridge.Class.define('Bridge.IFormatProvider',{});Bridge.Class.define('Bridge.ICloneable',{});Bridge.Class.generic('Bridge.IComparable$1',function(T){var $$name=Bridge.Class.genericName('Bridge.IComparable$1',T);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{}));});Bridge.Class.generic('Bridge.IEquatable$1',function(T){var $$name=Bridge.Class.genericName('Bridge.IEquatable$1',T);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{}));});Bridge.Class.define('Bridge.IPromise',{});

Bridge.Class.define("Bridge.DateTimeFormatInfo",{extend:[Bridge.IFormatProvider,Bridge.ICloneable],statics:{$allStandardFormats:{"d":"shortDatePattern","D":"longDatePattern","f":"longDatePattern shortTimePattern","F":"longDatePattern longTimePattern","g":"shortDatePattern shortTimePattern","G":"shortDatePattern longTimePattern","m":"monthDayPattern","M":"monthDayPattern","o":"roundtripFormat","O":"roundtripFormat","r":"rfc1123","R":"rfc1123","s":"sortableDateTimePattern","t":"shortTimePattern","T":"longTimePattern","u":"universalSortableDateTimePattern","U":"longDatePattern longTimePattern","y":"yearMonthPattern","Y":"yearMonthPattern"},constructor:function(){this.invariantInfo=Bridge.merge(new Bridge.DateTimeFormatInfo(),{abbreviatedDayNames:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],abbreviatedMonthGenitiveNames:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec",""],abbreviatedMonthNames:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec",""],amDesignator:"AM",dateSeparator:"/",dayNames:["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],firstDayOfWeek:0,fullDateTimePattern:"dddd, MMMM dd, yyyy h:mm:ss tt",longDatePattern:"dddd, MMMM dd, yyyy",longTimePattern:"h:mm:ss tt",monthDayPattern:"MMMM dd",monthGenitiveNames:["January","February","March","April","May","June","July","August","September","October","November","December",""],monthNames:["January","February","March","April","May","June","July","August","September","October","November","December",""],pmDesignator:"PM",rfc1123:"ddd, dd MMM yyyy HH':'mm':'ss 'GMT'",shortDatePattern:"M/d/yyyy",shortestDayNames:["Su","Mo","Tu","We","Th","Fr","Sa"],shortTimePattern:"h:mm tt",sortableDateTimePattern:"yyyy'-'MM'-'dd'T'HH':'mm':'ss",timeSeparator:":",universalSortableDateTimePattern:"yyyy'-'MM'-'dd HH':'mm':'ss'Z'",yearMonthPattern:"MMMM, yyyy",roundtripFormat:"yyyy'-'MM'-'dd'T'HH':'mm':'ss.uzzz"});}},getFormat:function(type){switch(type){case Bridge.DateTimeFormatInfo:return this;default:return null;}},getAbbreviatedDayName:function(dayofweek){if(dayofweek<0||dayofweek>6){throw new Bridge.ArgumentOutOfRangeException("dayofweek");}
...
var df=(provider||Bridge.CultureInfo.getCurrentCulture()).getFormat(Bridge.DateTimeFormatInfo),am=df.amDesignator,pm=df.pmDesignator,int=0,index=0,i,c,token,year=0,month=1,date=1,hh=0,mm=0,ss=0,ff=0,tt="",zzh=0,zzm=0,zzi,sign,neg,names,name,invalid=false,inQuotes=false;if(str==null){throw new Bridge.ArgumentNullException("str");}
format=format||"G";if(format.length==1){formats=df.getAllDateTimePatterns(format,true);format=formats?formats[0]:format;}
else if(format.length==2&&format.charAt(0)=="%"){format=format.charAt(1);}
while(index<format.length){c=format.charAt(index);token="";while((format.charAt(index)==c)&&(index<format.length)){token+=c;index++}
if(token=="yyyy"||token=="yy"||token=="y"){if(token=="yyyy"){year=this.subparseInt(str,int,4,4);}
else if(token=="yy"){year=this.subparseInt(str,int,2,2);}
else if(token=="y"){year=this.subparseInt(str,int,2,4);}
if(year==null){invalid=true;break;}
int+=year.length;if(year.length==2){year=~~year;year=(year>30?1900:2000)+year;}}
...
for(i=0;i<names.length;i++){name=names[i];if(str.substring(int,int+name.length).toLowerCase()==name.toLowerCase()){month=(i%12)+1;int+=name.length;break;}}
if((month<1)||(month>12)){invalid=true;break;}}
else if(token=="MM"||token=="M"){month=this.subparseInt(str,int,token.length,2);if(month==null||month<1||month>12){invalid=true;break;}
int+=month.length;}
else if(token=="dddd"||token=="ddd"){names=token==="ddd"?df.abbreviatedDayNames:df.dayNames;for(i=0;i<names.length;i++){name=names[i];if(str.substring(int,int+name.length).toLowerCase()==name.toLowerCase()){int+=name.length;break;}}}
else if(token=="dd"||token=="d"){date=this.subparseInt(str,int,token.length,2);if(date==null||date<1||date>31){invalid=true;break;}
int+=date.length;}
else if(token=="hh"||token=="h"){hh=this.subparseInt(str,int,token.length,2);if(hh==null||hh<1||hh>12){invalid=true;break;}
int+=hh.length;}
else if(token=="HH"||token=="H"){hh=this.subparseInt(str,int,token.length,2);if(hh==null||hh<0||hh>23){invalid=true;break;}
int+=hh.length;}
else if(token=="mm"||token=="m"){mm=this.subparseInt(str,int,token.length,2);if(mm==null||mm<0||mm>59){return null;}
int+=mm.length;}
else if(token=="ss"||token=="s"){ss=this.subparseInt(str,int,token.length,2);if(ss==null||ss<0||ss>59){invalid=true;break;}
int+=ss.length;}
else if(token=="u"){ff=this.subparseInt(str,int,1,7);if(ff==null){invalid=true;break;}
int+=ff.length;if(ff.length>3){ff=ff.substring(0,3);}}
else if(token=="fffffff"||token=="ffffff"||token=="fffff"||token=="ffff"||token=="fff"||token=="ff"||token=="f"){ff=this.subparseInt(str,int,token.length,7);if(ff==null){invalid=true;break;}
int+=ff.length;if(ff.length>3){ff=ff.substring(0,3);}}
else if(token=="t"){if(str.substring(int,int+1).toLowerCase()==am.charAt(0).toLowerCase()){tt=am;}
else if(str.substring(int,int+1).toLowerCase()==pm.charAt(0).toLowerCase()){tt=pm;}
else{invalid=true;break;}
int+=1;}
else if(token=="tt"){if(str.substring(int,int+2).toLowerCase()==am.toLowerCase()){tt=am;}
else if(str.substring(int,int+2).toLowerCase()==pm.toLowerCase()){tt=pm;}
else{invalid=true;break;}
int+=2;}
else if(token=="z"||token=="zz"){sign=str.charAt(int);if(sign=="-"){neg=true;}
else if(sign=="+"){neg=false;}
else{invalid=true;break;}
int++;zzh=this.subparseInt(str,int,1,2);if(zzh==null||zzh>14){invalid=true;break;}
int+=zzh.length;if(neg){zzh=-zzh;}}
else if(token=="zzz"){name=str.substring(int,int+6);int+=6;if(name.length!=6){invalid=true;break;}
...
else{name=str.substring(int,int+token.length);if((!inQuotes&&((token==":"&&name!=df.timeSeparator)||(token=="/"&&name!=df.dateSeparator)))||(name!=token&&token!="'"&&token!='"'&&token!="\\")){invalid=true;break;}
if(inQuotes=="\\"){inQuotes=false;}
if(token!="'"&&token!='"'&&token!="\\"){int+=token.length;}
else{if(inQuotes===false){inQuotes=token;}
else{if(inQuotes!=token){invalid=true;break;}
inQuotes=false;}}}}
if(inQuotes){invalid=true;}
if(!invalid){if(int!=str.length){invalid=true;}
...
Bridge.Class.define('Bridge.TimeSpan',{extend:[Bridge.IComparable],statics:{fromDays:function(value){return new Bridge.TimeSpan(value*864e9);},fromHours:function(value){return new Bridge.TimeSpan(value*36e9);},fromMilliseconds:function(value){return new Bridge.TimeSpan(value*1e4);},fromMinutes:function(value){return new Bridge.TimeSpan(value*6e8);},fromSeconds:function(value){return new Bridge.TimeSpan(value*1e7);},fromTicks:function(value){return new Bridge.TimeSpan(value);},constructor:function(){this.zero=new Bridge.TimeSpan(0);this.maxValue=new Bridge.TimeSpan(864e13);this.minValue=new Bridge.TimeSpan(-864e13);},getDefaultValue:function(){return new Bridge.TimeSpan(0);}},$config:function(){return{fields:{ticks:0}};},constructor:function(){if(arguments.length==1){this.ticks=arguments[0];}
...
Bridge.Class.define('Bridge.Text.StringBuilder',{statics:{},$config:function(){return{fields:{buffer:[]}}},constructor:function(){if(arguments.length==1){this.append(arguments[0]);}
...
var s=this.buffer.join('');this.buffer=[];this.buffer[0]=s;return s.length;},toString:function(){var s=this.buffer.join('');this.buffer=[];this.buffer[0]=s;if(arguments.length==2){var startIndex=arguments[0],length=arguments[1];this.checkLimits(s,startIndex,length);return s.substr(startIndex,length);}
...
if(length>value.length-startIndex){throw new ArgumentOutOfRangeException("Index and length must refer to a location within the string");}}});
...
Bridge.Class.define('Bridge.IEnumerable',{});Bridge.Class.define('Bridge.IEnumerator',{});Bridge.Class.define('Bridge.IEqualityComparer',{});Bridge.Class.define('Bridge.ICollection',{extend:[Bridge.IEnumerable]});Bridge.Class.generic('Bridge.IEnumerator$1',function(T){var $$name=Bridge.Class.genericName('Bridge.IEnumerator$1',T);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{extend:[Bridge.IEnumerator]}));});Bridge.Class.generic('Bridge.IEnumerable$1',function(T){var $$name=Bridge.Class.genericName('Bridge.IEnumerable$1',T);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{extend:[Bridge.IEnumerable]}));});Bridge.Class.generic('Bridge.ICollection$1',function(T){var $$name=Bridge.Class.genericName('Bridge.ICollection$1',T);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{extend:[Bridge.IEnumerable$1(T)]}));});Bridge.Class.generic('Bridge.IEqualityComparer$1',function(T){var $$name=Bridge.Class.genericName('Bridge.IEqualityComparer$1',T);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{extend:[Bridge.IEqualityComparer]}));});Bridge.Class.generic('Bridge.IDictionary$2',function(TKey,TValue){var $$name=Bridge.Class.genericName('Bridge.IDictionary$2',TKey,TValue);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{extend:[Bridge.IEnumerable$1(Bridge.KeyValuePair$2(TKey,TValue))],}));});

Bridge.Class.define("Bridge.CustomEnumerator",{extend:[Bridge.IEnumerator],constructor:function(moveNext,getCurrent,reset,dispose,scope){this.$moveNext=moveNext;this.$getCurrent=getCurrent;this.$dispose=dispose;this.$reset=reset;this.scope=scope;},moveNext:function(){try{return this.$moveNext.call(this.scope);}
...
Bridge.Class.define('Bridge.ArrayEnumerator',{constructor:function(array){this.array=array;this.reset();},moveNext:function(){this.index++;return this.index<this.array.length;},getCurrent:function(){return this.array[this.index];},reset:function(){this.index=-1;}});

Bridge.Class.generic('Bridge.EqualityComparer$1',function(T){var $$name=Bridge.Class.genericName('Bridge.EqualityComparer$1',T);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{extend:[Bridge.IEqualityComparer$1(T)],equals:function(x,y){if(!Bridge.isDefined(x,true)){return!Bridge.isDefined(y,true);}
...
Bridge.Class.generic('Bridge.KeyValuePair$2',function(TKey,TValue){var $$name=Bridge.Class.genericName('Bridge.KeyValuePair$2',TKey,TValue);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{constructor:function(key,value){this.key=key;this.value=value;}}));});Bridge.Class.generic('Bridge.Dictionary$2',function(TKey,TValue){var $$name=Bridge.Class.genericName('Bridge.Dictionary$2',TKey,TValue);return Bridge.Class.cache[$$name]||(Bridge.Class.cache[$$name]=Bridge.Class.define($$name,{extend:[Bridge.IDictionary$2(TKey,TValue)],constructor:function(obj,comparer){this.comparer=comparer||Bridge.EqualityComparer$1.default;this.clear();if(Bridge.is(obj,Bridge.Dictionary$2(TKey,TValue))){var e=Bridge.getEnumerator(obj),c;while(e.moveNext()){c=e.getCurrent();this.add(c.key,c.value);}}
...
Bridge.Class.define('Bridge.INotifyPropertyChanged',{});Bridge.Class.define('Bridge.PropertyChangedEventArgs',{constructor:function(propertyName){this.propertyName=propertyName;}});

Bridge.Array={toIndex:function(arr,indices){if(indices.length!=(arr.$s?arr.$s.length:1)){throw new Bridge.ArgumentException("Invalid number of indices");}
...
return idx;},get:function(indices){var r=this[Bridge.Array.toIndex(this,indices)];return typeof r!=="undefined"?r:this.$v;},set:function(indices,value){this[Bridge.Array.toIndex(this,indices)]=value;},create:function(defvalue,initValues,sizes){var arr=[],length=arguments[2],i,s,v,idx,indices,flatIdx;arr.$v=defvalue;arr.$s=[];arr.get=Bridge.Array.get;arr.set=Bridge.Array.set;for(i=2;i<arguments.length;i++){length*=arguments[i];arr.$s[i-2]=arguments[i];}
