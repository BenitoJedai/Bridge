...
if(config.init){config.init.apply(this,arguments);}};},define:function(className,prop){prop=prop||{};var extend=prop.$inherits||prop.inherits,statics=prop.$statics||prop.statics,base=extend?extend[0].prototype:this.prototype,prototype,nameParts,scope=prop.$scope||Bridge.global,i,v,ctorCounter,isCtor,name;if(Bridge.isFunction(extend)){extend=null;}
...
var delayInit=function(){if(Class.$initMembers){Class.$initMembers.call(Class);}
if(Class.constructor){Class.constructor.call(Class);}};setTimeout(delayInit,0);return Class;},addExtend:function(cls,extend){Array.prototype.push.apply(cls.$$inherits,extend);for(i=0;i<extend.length;i++){scope=extend[i];if(!scope.$$inheritors){scope.$$inheritors=[];}
