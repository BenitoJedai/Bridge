...
Bridge.Class.define('Bridge.Text.StringBuilder',{statics:{},config:{fields:{buffer:[]}},constructor:function(){if(arguments.length==1){this.append(arguments[0]);}
else if(arguments.length==3){this.append(arguments[0],arguments[1],arguments[2]);}},length:function(){if(this.buffer.length<2){return(this.buffer[0])?this.buffer[0].length:0;}
...
if(arguments.length==2){var count=arguments[1];if(count==0)
return this;else if(count<0){throw new Bridge.ArgumentOutOfRangeException("count","cannot be less than zero");}
value=Array(count+1).join(value).toString();}
else if(arguments.length==3){var startIndex=arguments[1],count=arguments[2];if(count==0)
return this;this.checkLimits(value,startIndex,count);value=value.substr(startIndex,count);}
...
return this.append("\r\n");},equals:function(sb){if(sb==null)
return false;if(sb==this)
return true;return(this.toString()===sb.toString());},remove:function(startIndex,length){var s=this.buffer.join('');this.checkLimits(s,startIndex,length);if(s.length==length&&startIndex==0){return this.clear();}
...
if(arguments.length==3){var count=arguments[2];if(count==0)
return this;else if(count<0){throw new Bridge.ArgumentOutOfRangeException("count","cannot be less than zero");}
value=Array(count+1).join(value).toString();}
...
return this;},replace:function(oldValue,newValue){var r=new RegExp(oldValue,'g'),s=this.buffer.join('');this.buffer=[];if(arguments.length==4){var startIndex=arguments[2],count=arguments[3];this.checkLimits(s,startIndex,count);var b=s.substr(startIndex,count);this.buffer[0]=s.substring(0,startIndex);this.buffer[1]=b.replace(r,newValue);this.buffer[2]=s.substring(startIndex+count,s.length);}
