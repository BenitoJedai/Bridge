...
if(config.properties){for(name in config.properties){this[name]=config.properties[name];var cap=name.charAt(0).toUpperCase()+name.slice(1);this["get"+cap]=(function(name){return function(){return this[name];};})(name);this["set"+cap]=(function(name){return function(value){this[name]=value;};})(name);}}
if(config.events){for(name in config.events){this[name]=config.events[name];this["add"+name]=(function(name){return function(value){this[name]=Bridge.fn.combine(this[name],value);};})(name);this["remove"+name]=(function(name){return function(value){this[name]=Bridge.fn.remove(this[name],value);};})(name);}}
...
return this.abbreviatedMonthNames[month-1];},getAllDateTimePatterns:function(format){var f=Bridge.DateTimeFormatInfo.$allStandardFormats,formats,names,pattern,i,result=[];if(format){if(!f[format]){throw new Bridge.ArgumentException(null,"format");}
...
f=endIndex;}else if(c=="\\"){buffer+=format.charAt(f+1);f++;}else if(c=="#"||c=="0"){groupCfg.buffer=buffer;if(i<integralDigits){if(i>=0){if(unused){this.groupedAppend(number.substr(0,i),groupCfg);}
this.groupedAppend(number.charAt(i),groupCfg);}else if(i>=integralDigits-forcedDigits){this.groupedAppend("0",groupCfg);}
unused=0;}else if(forcedDecimals-->0||i<number.length){this.groupedAppend(i>=number.length?"0":number.charAt(i),groupCfg);}
...
return buffer;},groupedAppend:function(value,cfg){var buffer=cfg.buffer,sep=cfg.sep,groupIndex=cfg.groupIndex;for(var i=0,length=value.length;i<length;i++){buffer+=value.charAt(i);if(sep&&groupIndex>1&&groupIndex--%3==1){buffer+=sep;}}
...
Bridge.Date={today:function(){var d=new Date();return new Date(d.getFullYear(),d.getMonth(),d.getDate());},format:function(date,format){throw new Bridge.NotImplementedException();},parse:function(value,provider){throw new Bridge.NotImplementedException();},parseExact:function(value,format,provider){throw new Bridge.NotImplementedException();},tryParse:function(value,provider,result){throw new Bridge.NotImplementedException();},tryParseExact:function(value,format,provider,result){throw new Bridge.NotImplementedException();},isDaylightSavingTime:function(dt){var temp=Bridge.Date.today();temp.setMonth(0);temp.setDate(1);return temp.getTimezoneOffset()!=dt.getTimezoneOffset();},toUTC:function(date){return new Date(date.getUTCFullYear(),date.getUTCMonth(),date.getUTCDate(),date.getUTCHours(),date.getUTCMinutes(),date.getUTCSeconds(),date.getUTCMilliseconds());},toLocal:function(date){return new Date(Date.UTC(date.getFullYear(),date.getMonth(),date.getDate(),date.getHours(),date.getMinutes(),date.getSeconds(),date.getMilliseconds()));}};
