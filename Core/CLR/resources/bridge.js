...
return false;},get:function(key){var entry=this.findEntry(key);if(!entry){throw new Error('Key not found: '+key);}
return entry.value;},set:function(key,value,add){var entry=this.findEntry(key),hash;if(entry){if(add){throw new Error('Key already exists: '+key);}
...
result=new Array(tasks.length);for(i=0;i<tasks.length;i++){tasks[i].$index=i;tasks[i].continueWith(function(t){switch(t.status){case Bridge.TaskStatus.ranToCompletion:result[t.$index]=t.getResult();break;case Bridge.TaskStatus.canceled:cancelled=true;break;case Bridge.TaskStatus.faulted:errors.push(t.error);break;default:throw new Error('Invalid task status: '+t.status);}
