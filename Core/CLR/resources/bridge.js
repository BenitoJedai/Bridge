...
return names;},isDefined:function(value){return typeof value!=='undefined';},toArray:function(ienumerable){var i,item,len,result=[];if(Bridge.isArray(ienumerable)){for(i=0,len=ienumerable.length;i<len;++i){result.push(ienumerable[i]);}}
...
Bridge.Class.extend('Bridge.IEnumerable',{});Bridge.Class.extend('Bridge.IEnumerator',{});Bridge.Class.extend('Bridge.Dictionary',{$extend:[Bridge.IEnumerable],$init:function(obj){if(Object.prototype.toString.call(obj)==='[object Object]'){this.entries=obj;this.count=Bridge.getPropertyNames(this.entries).length;}
else if(Bridge.is(obj,Bridge.Dictionary)){this.entries={};this.count=0;for(var key in obj){this.entries[key]=obj[key];this.count++;}}
...
catch(e){task.setError(e);}},0);return task;},whenAll:function(tasks){var task=new Bridge.Task(),result,executing=tasks.length,cancelled=false,errors=[],i;if(tasks.length===0){task.setResult([]);return task;}
...
return task;},whenAny:function(tasks){if(!tasks.length){throw new Error('At least one task is required');}
